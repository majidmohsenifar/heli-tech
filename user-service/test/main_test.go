...
func truncateDB() error {
	ctx := context.Background()
	db := getDB()
	_, err := db.Exec(ctx, "TRUNCATE TABLE roles_routes CASCADE")
	if err != nil {
		return err
	}
	_, err = db.Exec(ctx, "TRUNCATE TABLE users_roles CASCADE")
	if err != nil {
		return err
	}
	_, err = db.Exec(ctx, "TRUNCATE TABLE routes CASCADE")
	if err != nil {
		return err
	}
	_, err = db.Exec(ctx, "TRUNCATE TABLE roles CASCADE")
	if err != nil {
		return err
	}
